...
##' @title Compute the CLUSCATA partitioning algorithm on different blocks from a CATA experiment
##' @description
...
cluscata_kmeans <- function(Data, nblo, clust, nstart = 100, rho = 0, NameBlocks = NULL, NameVar = NULL, Itermax = 30,
  ...
  names(AFC) <- names(scomp) <- names(alpha) <- names(rho) <- names(inertia) <- paste("Cluster", 1:ngroups)
  names(cr) <- NameBlocks

  # results
  res <- list(
    group = grou, rho = rho, homogeneity = tab1, s_with_compromise = scomp, weights = alpha,
